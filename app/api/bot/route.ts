...
bot.on(['text', 'photo'], async (ctx: any) => {
  ...
  if (state.step === 'MATERIAL') {
    // Basic validation to ensure they picked from list (optional)
  ...
  if (state.step === 'PRICE') {
    ...
    // SEND SUMMARY TO ADMIN NOW (Before photos start coming)
    const summary = `
...
💰 **Price:** ${text} ETB
  ...
  if (state.step === 'PHOTO') {
    if (ctx.message.photo) {
      // Forward the photo immediately to Admin
